...
std::vector<float> calculate_probabilities(std::vector<unsigned int> &kmer_counts, std::vector<unsigned int> &species_counts, float alpha) {
    std::vector<float> probabilities;
    probabilities.reserve(kmer_counts.size());
    float probability_sum = 0;
    for (size_t i = 0; i < kmer_counts.size(); ++i) {
        float probability = (kmer_counts[i] + alpha) / (species_counts[i] + species_counts.size() * alpha);
